...
})({"ih113":[function(require,module,exports) {
var global = arguments[3];
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "3beabdf0e20d2917";
"use strict";
...
},{}],"jXca2":[function(require,module,exports) {
//const n = 100;
const DI = 70;
//size40
const OB = 86;
const VB = 18;
const OT = 60;
var height = 1000, width = 1000, margin = 100;
var svg = d3.select("body").append("svg").attr("class", "axis").attr("width", width).attr("height", height);
var xAxisLenght = width - 2 * margin;
var yAxisLength = height - 2 * margin;
var scaleX = d3.scale.linear().domain([
    0,
    100
]).range([
    0,
...
var scaleY = d3.scale.linear().domain([
    0,
    100
]).range([
    0,
...
svg.append("g").attr("class", "x-axis").attr("transform", "translate(100,100)").call(xAxis);
...
d3.selectAll("g.y-axis g.tick").append("line").classed("grid-line", true).attr("x1", 0).attr("y1", 0).attr("x2", xAxisLenght).attr("y2", 0);
// координаты сетки
let A = {
    x: 0,
    y: 0
...
let C1 = {
    x: (0.5 * OB + 1) * 0.5 - side_additions / 2,
    y: -1
};
let C2 = {
    x: (0.5 * OB + 1) * 0.5 + side_additions / 2,
...
//
for(i = 0; i < rawData.length; i++)data.push({
...
var g = svg.append("g");
g.append("path").attr("d", line(data)).style("stroke", "steelblue").style("stroke-width", 2);
// добавляем отметки к точкам
svg.selectAll(".dot").data(rawData).enter().append("circle").attr("class", "dot").attr("r", 5.5).attr("cx", function(d) {
...
function drawLines(coor) {
    rD = coor, d1 = [];
    for(n = 0; n < rD.length; n++)d1.push({
        x: scaleX(rD[n].x) + margin,
        y: scaleY(rD[n].y) + margin
    });
    ///
    var line = d3.svg.line().x(function(d) {
        return d.x;
    ...
    var g = svg.append("g");
    g.append("path").attr("d", line(d1)).style("stroke", "steelblue").style("stroke-width", 2);
    // добавляем отметки к точкам
    svg.selectAll(".dot").data(rD).enter().append("circle").attr("class", "dot").attr("r", 5.5).attr("cx", function(d) {
        return scaleX(d.x) + margin;
    }).attr("cy", function(d) {
        return scaleY(d.y) + margin;
    });
...
},{}]},["ih113","jXca2"], "jXca2", "parcelRequire94c2")

//# sourceMappingURL=main.e20d2917.js.map
